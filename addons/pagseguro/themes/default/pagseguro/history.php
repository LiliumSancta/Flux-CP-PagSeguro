...
<table class="vertical-table">
	...
	<tr>
		<td colspan="4" style="color:Orange">
			↳ Sua doação possui status de (<?php $status = Flux::config('PagSeguroStatus')->toArray();
			echo htmlspecialchars($status[$pendente->payment_status]) ?>) no PagSeguro.
		</td>
	</tr>
	<?php endforeach ?>
</table>
...
<table class="vertical-table">
	...
	<tr>
		<td colspan="4" style="color:MediumSeaGreen">
			↳ Sua doação possui status de (<?php $status = Flux::config('PagSeguroStatus')->toArray();
			echo htmlspecialchars($status[$aprovada->payment_status]) ?>) no PagSeguro.
		</td>
	</tr>
	<?php endforeach ?>
</table>
...
<table class="vertical-table">
	...
	<tr>
		<td colspan="4" style="color:Tomato">
			↳ Sua doação possui status de (<?php $status = Flux::config('PagSeguroStatus')->toArray();
			echo htmlspecialchars($status[$recusada->payment_status]) ?>) no PagSeguro.
		</td>
	</tr>
	<?php endforeach ?>
</table>
